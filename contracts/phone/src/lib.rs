...
impl Phone {
    ...
    pub fn create_new_phone_book(env: Env, phone_book_name: String, salt: BytesN<32>, wasm_hash: BytesN<32>, constructor_args: Vec<Val>,) {
        let owner: Address = env.storage().instance().get(&PhoneKey::Owner).expect("msg");
        owner.require_auth();
        let phone_book_address: Address = env.deployer().with_address(owner.clone(), salt).deploy_v2(wasm_hash, constructor_args);

        // save it 
        env.storage().instance().set(&PhoneKey::PhoneBookName(phone_book_name), &phone_book_address);
    }
    ...
    pub fn add_new_contact(env: Env, phone_book_name: String, contact_info: ContactInfo ) {
        ...
        let phone_book_address: Address = env.storage().instance().get(&PhoneKey::PhoneBookName(phone_book_name)).expect("msg");
    ...
    pub fn get_contact(env: Env, phone_book_name: String, contact_name: String) -> Option<phone_book::ContactInfo>{
        let owner: Address = env.storage().instance().get(&PhoneKey::Owner).expect("msg");
        owner.require_auth();
